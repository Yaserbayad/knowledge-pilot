...
test('production reading shell remains valid JavaScript', () => {
  const shell = fileURLToPath(new URL('../public/reading-shell.js', import.meta.url));
  const a11y = fileURLToPath(new URL('../public/reading-shell-a11y.js', import.meta.url));
  assert.doesNotThrow(() => execFileSync(process.execPath, ['--check', shell], { stdio: 'pipe' }));
  assert.doesNotThrow(() => execFileSync(process.execPath, ['--check', a11y], { stdio: 'pipe' }));
});
...
test('learner shell loads the frozen reading experience through external strict-CSP assets', async () => {
  ...
  assert.match(html, /<script type="module" src="\/assets\/reading-shell-a11y\.js"><\/script>/);
...
test('full-screen reader removes the underlying workspace from keyboard focus and restores focus on exit', async () => {
  const js = await read('../public/reading-shell-a11y.js');
  assert.match(js, /topbar\.inert = value/);
  assert.match(js, /main\.inert = value/);
  assert.match(js, /readerReady/);
  assert.match(js, /reading-hero h1/);
  assert.match(js, /heading\.tabIndex = -1/);
  assert.match(js, /returnFocus/);
  assert.match(js, /target\.focus/);
  assert.match(js, /removeAttribute\('aria-live'\)/);
});
...
test('service worker caches only the new static reader assets, never reader API payloads', async () => {
  ...
  assert.match(sw, /\/assets\/reading-shell-a11y\.js/);
