...
function normalizePlanVerification(value) {
  const verification = value && typeof value === 'object' ? value : {};
  return {
    learnerFit: boundedText(verification.learnerFit, 2000),
    noveltyCheck: boundedText(verification.noveltyCheck, 2000),
    coherenceCheck: boundedText(verification.coherenceCheck, 2000)
  };
}

function normalizeLessonVerification(value) {
  const verification = value && typeof value === 'object' ? value : {};
  ...
  const finalAudit = verification.finalAudit && typeof verification.finalAudit === 'object'
    ...
    : {};
  return {
    researchApproach: boundedText(verification.researchApproach, 3000),
    consensusStatus: ['established', 'mixed', 'uncertain'].includes(verification.consensusStatus)
      ? verification.consensusStatus
      : 'uncertain',
    disagreements: boundedList(verification.disagreements, 12, 1000),
    uncertainty: boundedList(verification.uncertainty, 12, 1000),
    adversarialReview: {
      issuesFound: boundedList(adversarialReview.issuesFound, 12, 1000),
      correctionsMade: boundedList(adversarialReview.correctionsMade, 12, 1000),
      unresolvedIssues: boundedList(adversarialReview.unresolvedIssues, 12, 1000)
    },
    finalAudit: {
...
export function normalizePlan(raw, user) {
  ...
  if (raw && typeof raw === 'object') raw.verification = normalizePlanVerification(raw.verification);
  return {
...
export function normalizeLesson(raw, proposal, sources, user) {
  ...
  if (raw && typeof raw === 'object') raw.verification = normalizeLessonVerification(raw.verification);
  const aiSourceAnnotations = new Map((Array.isArray(raw.sources) ? raw.sources : []).slice(0, 50)
    .map((source) => [boundedText(source.id, 120), source]));
